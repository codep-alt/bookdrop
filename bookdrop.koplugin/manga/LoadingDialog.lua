...
function LoadingDialog:showAndRunWithProgress(message, runnable, onCancel, onConfirmCancel)
  ...
  local shm_ok = raw_shm ~= ffi.cast("void *", -1)

  local shm
  if shm_ok then
    shm = ffi.cast("ko_progress_shm_t *", raw_shm)
    shm.processed = 0
    shm.total = 0
    shm.updated = 0
  end
  ...
  if shm_ok then
    ...
    -- cleanup
    if poll_action then UIManager:unschedule(poll_action) end
    ...
    -- emulator / macOS fallback: no shared memory → run synchronously
    UIManager:close(message_dialog)
    local ok, result = pcall(runnable, function() end)
